package net.orhanbalci.fuzzymoea.operator;
...
public class ScrambleMutation<T> implements MutationOperator<PermutationSolution<T>> {
  ...
  /** Constructor */
  public ScrambleMutation(double mutationProbability, RandomGenerator<Double> randomGenerator) {
    this(
        mutationProbability,
        randomGenerator,
  ...
  public void doMutation(PermutationSolution<T> solution) {
    ...
    if ((permutationLength != 0) && (permutationLength != 1)) {
      if (mutationRandomGenerator.getRandomValue() < mutationProbability) {
        ...
        List<T> interval = new ArrayList<T>();
        for (int i = pos1; i < pos2; i++) {
          interval.add(solution.getVariableValue(i));
        }
        Collections.shuffle(interval);
        for (int i = pos1; i < pos2; i++) {
          solution.setVariableValue(i, interval.get(i - pos1));
        }
      }
    }
  }
